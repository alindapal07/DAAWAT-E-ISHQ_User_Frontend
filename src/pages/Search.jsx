...
const parseNaturalQuery = (query) => {
  ...
  const hints = { maxPrice: null, minRating: null, dietary: null, isSpicy: null, keywords: [] };

  const priceMatch = q.match(/under\s*(?:rs\.?|₹)?\s*(\d+)/i) || q.match(/(?:below|less than)\s*(?:rs\.?|₹)?\s*(\d+)/i);
  if (priceMatch) hints.maxPrice = parseFloat(priceMatch[1]);

  if (/\b(veg|vegetarian)\b/.test(q)) hints.dietary = 'veg';
  if (/\b(non.?veg|chicken|mutton|fish|prawn|egg)\b/.test(q)) hints.dietary = 'non-veg';
  if (/\b(spicy|hot|fiery)\b/.test(q)) hints.isSpicy = true;
  if (/\b(healthy|light|salad|soup)\b/.test(q)) hints.minRating = 4.0;

  const stopWords = new Set(['under','below','less','than','rs','for','me','some','the','a','an','with','and','or','in','near','good','best','find','show','get']);
  hints.keywords = q.split(/\s+/).filter(w => !stopWords.has(w) && !/^\d+$/.test(w) && w.length > 1);
  return hints;
};
...
const renderStars = (rating) =>
  Array.from({ length: 5 }, (_, i) => (
    <Star key={i} size={13} fill={i < Math.round(rating) ? '#f59e0b' : '#d1d5db'} color={i < Math.round(rating) ? '#f59e0b' : '#d1d5db'} />
  ));
...
const SortDropdown = ({ value, onChange, options }) => {
  ...
  return (
    <div className="custom-sort-dropdown" ref={dropdownRef}>
      ...
      <ul className={`sort-options-list${open ? ' sort-options-list--open' : ''}`} role="listbox">
        {options.map((option) => (
          <li
            key={option.value}
            role="option"
    ...
    <div className="result-content">
      <div className="result-header">
        ...
        <div className="result-rating">
          ...
          <span className="rating-number">{item.rating > 0 ? item.rating.toFixed(1) : ''}</span>
        </div>
      </div>
      <p className="result-description">{item.description}</p>
      <div className="result-meta">
        <div className="meta-item"><Clock size={13} /><span>{item.cookTime}</span></div>
        <div className="meta-item"><Users size={13} /><span>{item.type || item.category}</span></div>
      </div>
      <div className="result-tags">
        <span className={`tag ${item.isVeg ? 'vegetarian' : 'nonveg'}`}>{item.isVeg ? 'Veg' : 'Non-Veg'}</span>
        {item.isSpicy && <span className="tag spice-hot">Spicy</span>}
      </div>
      <div className="result-footer">
        <div className="price">Rs.{item.price}</div>
        <button onClick={() => onAddToCart(item)} className="add-to-cart-btn">Add to Cart</button>
      </div>
    </div>
  </div>
);
...
const RecommendedSection = ({ items, onAddToCart }) => {
  ...
  const trending = useMemo(() =>
    [...items]
      .filter(i => i.tags && i.tags.some(t => ['Bestseller', 'Popular', 'Trending', 'Chef Special', 'Signature Dish'].includes(t)))
      .slice(0, 4),
    [items]
  );
    ...
    <div className="empty-state">
      ...
      <div className="empty-suggestions">
        ...
        <div className="empty-chips">
          {tips.map(tip => (
            <button key={tip} className="empty-chip" onClick={() => onSuggestionClick(tip)}>
              {tip}
            </button>
          ))}
        </div>
...
const SearchPage = () => {
  ...
  const [filters, setFilters] = useState({ category: '', priceRange: '', rating: '', dietary: '' });
    ...
    let filtered = menuItems.filter((item) => {
      ...
      if (searchQuery.trim()) {
        ...
        matchesQuery = words.every((word) => {
          const loose = word === 'biriyani' ? 'biryani' : word;
          return (
            item.name.toLowerCase().includes(loose) ||
            (item.description && item.description.toLowerCase().includes(loose)) ||
            (item.tags && item.tags.some((t) => t.toLowerCase().includes(loose))) ||
            (item.type && item.type.toLowerCase().includes(loose)) ||
            (item.category && item.category.toLowerCase().includes(loose))
          );
        });

        /* NLP: price hint */
        if (nlp.maxPrice && item.price > nlp.maxPrice) matchesQuery = false;
        if (nlp.isSpicy !== null && item.isSpicy !== nlp.isSpicy) matchesQuery = false;
        if (nlp.dietary === 'veg' && !item.isVeg) matchesQuery = false;
        if (nlp.dietary === 'non-veg' && item.isVeg) matchesQuery = false;
      }

      const matchesCategory = !filters.category || item.type === filters.category || item.category === filters.category;
      let matchesDietary = true;
      if (filters.dietary) matchesDietary = filters.dietary === 'veg' ? item.isVeg : !item.isVeg;
  ...
  const clearFilters = () => setFilters({ category: '', priceRange: '', rating: '', dietary: '' });
    ...
    <div className="search-page">
      <div className="search-hero">
        <div className="search-hero-inner">
          <div className="search-hero-text">
            ...
            <p>Search across our entire menu — try "Spicy chicken" or "Paneer under 200"</p>
          </div>

          {/* ── MAIN SEARCH BAR ── */}
          <div className="search-bar-wrap" ref={dropdownRef}>
            <form onSubmit={handleSearchSubmit} className="search-form-hero" autoComplete="off">
              <div className={`search-input-box${dropdownOpen ? ' focused' : ''}`}>
                ...
                {searchQuery && (
                  <button
                    ...
                    onClick={() => { setSearchQuery(''); setSearchParams({}); searchInputRef.current?.focus(); }}
                    ...
                    {suggestions.map(item => (
                      ...
                      >
                        ...
                        <div className="suggestion-info">
                          ...
                          <span className="suggestion-meta">{item.type || item.category} · Rs.{item.price}</span>
                        </div>
                    ...
                    {history.slice(0, 5).map(h => (
                      ...
                      >
                        ...
                        <div
                          ...
                          tabIndex="0"
                          aria-label={`Remove ${h}`}
                          onKeyDown={(e) => e.key === 'Enter' && handleDeleteHistory(h, e)}
                        >
                          <X size={12} />
                        </div>
                      </button>
          ...
          <div className="hero-quick-chips">
            {quickSearches.map(q => (
              <button key={q} className="hero-chip" onClick={() => handleSuggestionClick(q)}>{q}</button>
            ))}
          </div>
        </div>
      </div>
      ...
      <div className="container search-container">
        ...
        <div className="search-content">
          <div className={`filters-overlay${showFilters ? ' show' : ''}`} onClick={() => setShowFilters(false)} aria-hidden="true" />
          ...
          <aside className={`filters-sidebar${showFilters ? ' show' : ''}`}>
            <div className="filters-header">
              ...
              <div style={{display:'flex',gap:'0.5rem',alignItems:'center'}}>
                ...
                <button onClick={() => setShowFilters(false)} style={{background:'none',border:'none',cursor:'pointer',fontSize:'1.3rem',color:'#64748b',lineHeight:1}} aria-label="Close filters">×</button>
              </div>
            </div>
            <div className="filter-group">
              <h4>Category</h4>
              <select value={filters.category} onChange={(e) => setFilters((p) => ({...p,category:e.target.value}))} className="filter-select">
            ...
            <div className="filter-group">
              <h4>Dietary</h4>
              <select value={filters.dietary} onChange={(e) => setFilters((p) => ({...p,dietary:e.target.value}))} className="filter-select">
            ...
            <div className="filter-group">
              ...
              <select value={filters.priceRange} onChange={(e) => setFilters((p) => ({...p,priceRange:e.target.value}))} className="filter-select">
            ...
            <div className="filter-group">
              ...
              <select value={filters.rating} onChange={(e) => setFilters((p) => ({...p,rating:e.target.value}))} className="filter-select">
              ...
              <>
                ...
                <div className="results-grid">
                  {searchResults.map((item) => (
                    <ResultCard key={item.id} item={item} onAddToCart={addToCart} />
                  ))}
                </div>
              </>
            ) : (
              <EmptyState query={searchQuery} allItems={menuItems} onSuggestionClick={handleSuggestionClick} />
            )}
          </main>
        </div>
      </div>
